...
if __name__ == "__main__":
    ...
    train_x = [row[:col_index] + row[col_index + 1:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = dataset[int(0.85 * len(dataset)):]
    test_x = [row[:col_index] + row[col_index + 1:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]

    trees_n = int(input())
    criteria = input()
    classifier = RandomForestClassifier(criterion=criteria, n_estimators=trees_n, random_state=0)
