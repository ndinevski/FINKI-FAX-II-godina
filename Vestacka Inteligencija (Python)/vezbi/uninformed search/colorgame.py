...
class Colors(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(n):
            #color center square
            new_board = board[:]
            new_square = change_color(board[i])
            if new_square[0] in range(0,factor) and new_square[1] in range(0,factor):
                new_board[i] = new_square
            #gore
            if(i+factor < n):
                new_square = change_color(board[i+factor])
                if new_square[0] in range(0, factor) and new_square[1] in range(0, factor):
                    new_board[i+factor] = new_square
            #dolu
            if(i > factor):
                new_square = change_color(board[i-factor])
                if new_square[0] in range(0, factor) and new_square[1] in range(0, factor):
            ...
            #levo
            if(i > 0):
                new_square = change_color(board[i-1])
                if new_square[0] in range(0, factor) and new_square[1] in range(0, factor) and i%factor!=0:
                    new_board[i-1] = new_square
            #desno
            if(i+1 < n):
                new_square = change_color(board[i+1])
                if new_square[0] in range(0, factor) and new_square[1] in range(0, factor) and (i+1)%factor!=0:
                    new_board[i+1] = new_square

            succ["x: {}, y: {}".format(board[i][0], board[i][1])] = tuple(new_board)
...
if __name__ == "__main__":
    ...
    str = input().split(",")
    board = []
    goal_board = [(x,y,1) for x in range(n) for y in range(n)]
    ...
    colors = Colors(tuple(board), tuple(goal_board))

    print(breadth_first_graph_search(colors).solution())
