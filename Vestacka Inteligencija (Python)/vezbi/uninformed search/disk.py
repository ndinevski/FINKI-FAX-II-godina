...
class Disk(Problem):
    ...
    def __init__(self, initial, goal=None):
        ...
        self.goal_list = tuple(initial[0][::-1])
    ...
    def successor(self, state):
        ...
        for i in range(n):
            if lista[i+1]==0 and i<len(lista)-1:
                new_list[i],new_list[i+1] = new_list[i+1],new_list[i]
                successors["D1: Disk {}".format(i)] = (tuple(new_list), n)
            elif lista[i+2]==0 and i<len(lista)-2:
                new_list[i],new_list[i+2] = new_list[i+2],new_list[i]
                successors["D2: Disk {}".format(i)] = (tuple(new_list), n)
            elif lista[i-1]==0 and i>0:
                new_list[i], new_list[i-1] = new_list[i-1], new_list[i]
                successors["L1: Disk {}".format(i)] = (tuple(new_list),n)
            elif lista[i-2]==0 and i>1:
                new_list[i], new_list[i-2] = new_list[i-2], new_list[i]
                successors["L2: Disk {}".format(i)] = (tuple(new_list),n)
...
if __name__ == "__main__":
    ...
    for i in range(n):
        lista[i] = i+1
    disk = Disk((tuple(lista), n))
    print(breadth_first_tree_search(disk).solve())
