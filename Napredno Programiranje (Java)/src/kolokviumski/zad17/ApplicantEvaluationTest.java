...
class EvaluatorBuilder {
    public static <T> Evaluator<T> build(Evaluator.TYPE type) throws InvalidEvaluation {
        switch (type){
            case NO_CRIMINAL_RECORD:
                return p1 ->!p1.hasCriminalRecord();
            case MORE_EXPERIENCE:
                Evaluator<T> evaluator2 = p->p.getEmploymentYears()>10;
                return evaluator2;
            case MORE_CREDIT_SCORE:
                Evaluator<T> evaluator3 = p->p.getCreditScore()>500;
                return evaluator3;
            case NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE:
                return p1 ->(!p1.hasCriminalRecord() && p1.getEmploymentYears()>10);
            case MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE:
                Evaluator<T> evaluator5 = p->(p.getCreditScore()>500 && p.getEmploymentYears()>10);
                return evaluator5;
            case NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE:
                Evaluator<T> evaluator6 = p->(!p.hasCriminalRecord() && p.getCreditScore()>500);
                return evaluator6;
            case INVALID:
