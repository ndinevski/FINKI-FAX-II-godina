package laboratoriski.lab3.zad2;
...
class MaximumSizeExceddedException extends Exception{
    public MaximumSizeExceddedException(String mess) {
        super(mess);
    }
}
...
class Contact{
    ...
    public Contact(String name, String [] phonenumber) throws MaximumSizeExceddedException, InvalidNameException, InvalidNumberException {
        if(phonenumber.length > 5){
            throw new MaximumSizeExceddedException("Poveke od 5 broevi");
        }
        for(int i=0;i<name.length();i++){
            if(!Character.isDigit(name.charAt(i)) || !Character.isLetter(name.charAt(i)) || name.length()>10 || name.length()<4){
                throw new InvalidNameException("nevalidno ime");
            }
        }
        for(int i=0;i<phonenumber.length;i++){
            if(isNotValid(phonenumber[i])){
                throw new InvalidNumberException("nevaliden broj");
            }
        }
    ...
    boolean isNotValid(String phonenumber){
        for(int i=0;i< phonenumber.length();i++){
            if(phonenumber.charAt(0)!=0 || phonenumber.charAt(1)!=7 || phonenumber.charAt(2)==3 || phonenumber.charAt(2)==4 || phonenumber.charAt(2)==9){
                return true;
            }
    ...
    void addNumber(String phonenumber) throws InvalidNumberException, MaximumSizeExceddedException {
        ...
        String temp[] = Arrays.copyOf(numbers, numbers.length + 1);

        temp[numbers.length] = phonenumber;
