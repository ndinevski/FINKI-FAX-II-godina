package laboratoriski.kolokviumski.kol2.zad5;
...
public class Speluvanje {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<N;i++){
            ...
            Zbor zborO = new Zbor(zbor.toLowerCase());
            tabela.insert(zborO, zbor);
        }
        ...
        for(int i=0;i< text.length;i++){
            String wordToCheck = text[i];
            if(!Character.isLetter(wordToCheck.charAt(wordToCheck.length()-1))){
                wordToCheck = wordToCheck.substring(0,wordToCheck.length()-1);
            }

            Zbor zborToCheck = new Zbor(wordToCheck.toLowerCase());


            if(tabela.search(zborToCheck)==-1){
                sb.append(wordToCheck).append("\n");
                flag=1;
                continue;
            }
        }
