...
// дадени се два броеви, ID на првиот војник и ID на последниот војник од првиот интервал. Во четвртиот ред дадени се
// два броеви, ID на првиот војник и ID на последниот војник од вториот интервал.
...
// со целосно имплементиран input и output. Потребно е да се менува само во рамки на DLL<Integer> vojska(DLL<Integer>
// lista, int a, int b, int c, int d) функцијата.
...
public class DLLVojska {
    ...
    public static DLL<Integer> vojska(DLL<Integer> lista, int a, int b, int c, int d) {
        ...
        DLLNode<Integer> A = lista.getFirst();
        DLLNode<Integer> C = lista.getFirst();
        DLLNode<Integer> B = lista.getFirst();
        DLLNode<Integer> afterFirstInterval = new DLLNode<>(0,null,null);
        ...
        if(B.succ.equals(C)) {
            DLLNode<Integer> tmpp = C;
            while (tmpp.element != d) {
                lista.insertBefore(tmpp.element, A);
                tmpp = tmpp.succ;
                lista.delete(tmpp.pred);
            }
            lista.insertBefore(tmpp.element, A);
            lista.delete(tmpp);
            return lista;
        ...
        int flag=0, flag2=0;
        while(tmp!=null){
            if(tmp.element == a && flag==0){
                DLLNode<Integer> tmpp = tmp;
                while(tmpp.element != b){
                    lista.insertBefore(tmpp.element, C);
                    tmpp= tmpp.succ;
                    lista.delete(tmpp.pred);
                }
                lista.insertBefore(tmpp.element, C);
            ...
            if(tmp.element == c && flag2==0){
                DLLNode<Integer> tmpp = tmp;
                while(tmpp.element!=d){
                    lista.insertBefore(tmpp.element,afterFirstInterval);
                    tmpp= tmpp.succ;
                    lista.delete(tmpp.pred);
                }
                lista.insertBefore(tmpp.element, afterFirstInterval);
